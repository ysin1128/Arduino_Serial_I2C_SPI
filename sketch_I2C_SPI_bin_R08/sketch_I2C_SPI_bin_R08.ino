...
byte REV_Code[] = {0x00, 0x08}; // [0]: free, [1] Rev code of sketch
...
void CTRL_D62(byte byteDAT){
  int l;

  for(l=0;l<5;l++){
    CTRL_DIG_HL(2+l, CK_BIT(byteDAT, l));
  }
}

void CTRL_DIG_HL(int intDX, boolean selH){
  if(selH){
    digitalWrite(intDX, HIGH);
  }
  else{
    digitalWrite(intDX, LOW);
  }
}
